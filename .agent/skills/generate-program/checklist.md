...
> Resolve example paths through OpenCode's `@foldkit` reference first when available; otherwise use `repos/foldkit/` or the Foldkit project root. Paths beginning with `repos/foldkit/` refer to that resolved repository root.
