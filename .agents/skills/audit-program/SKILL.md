...
Resolve the **Foldkit repository root** before auditing. In OpenCode, use the `@foldkit` project reference first when available; its local path is advertised in the session's project references. Otherwise use `repos/foldkit/` when present, or the project root when auditing Foldkit itself. Only recommend adding the subtree when neither the OpenCode reference nor a local Foldkit repository is available. All repository paths below are relative to the resolved root.
...
These four files are a snapshot. The live source in the resolved Foldkit repository wins over anything here: read the `.d.ts` or the example rather than grading against a remembered signature.
...
4. **Tier-matching exemplar**. Pick at least one to read alongside the audited code:
   - Tier 1-2 → `examples/counter/src/main.ts`, `examples/stopwatch/src/main.ts`
   - Tier 3 → `examples/weather/src/main.ts`, `examples/form/src/main.ts`
   - Tier 4 → `examples/routing/src/main.ts`
   - Tier 5 → `examples/kanban/src/`
   - Tier 6 → `examples/auth/src/`, `examples/job-application/src/`
   - Tier 7 → `packages/typing-game/client/src/page/room/`
5. **Foldkit UI integration**. If anything is imported from `@foldkit/ui`, also read `examples/ui-showcase/src/main.ts` and `examples/ui-showcase/src/ui/` for the wiring pattern.
