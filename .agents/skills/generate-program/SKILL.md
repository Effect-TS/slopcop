...
### Resolve the Foldkit repository

Resolve the **Foldkit repository root** before following any repository path in this skill:

1. In OpenCode, use the `@foldkit` project reference first when it is available. Its local path is advertised in the session's project references. Do not ask the user to vendor Foldkit when this reference is available.
2. Otherwise use the project's `repos/foldkit/` subtree when present.
3. When working inside Foldkit itself, use the project root.

Repository paths such as `examples/weather/src/main.ts`, `packages/website/src/`, and legacy paths beginning with `repos/foldkit/` are relative to that resolved root; strip the `repos/foldkit/` prefix when necessary. Paths such as `architecture.md` are relative to this skill directory.
...
These four files are a snapshot of a moving codebase. When they disagree with the resolved Foldkit repository or the `.d.ts` in `node_modules`, the live source is right. Treat the disagreement as a bug in this skill and say so in your final report.
...
- `packages/typing-game/client/src/`: production multi-page app: Submodels, OutMessage, update/view decomposition, curried handler extraction, subscription patterns, domain modules.
- `packages/website/src/`: production Foldkit website: page organization, shared view primitives, route-driven rendering, idiomatic domain separation.
...
Read `examples/counter/src/main.ts`

**Tier 2: Timers, subscriptions, simple stateful apps:**
Read `examples/stopwatch/src/main.ts` (timer via subscription, `Duration` field pattern) and `examples/todo/src/main.ts` (CRUD with localStorage via flags)

**Tier 3: Async operations, loading/error states, API calls, form validation:**
Read `examples/weather/src/main.ts` (HTTP with `HttpClient`) and `examples/form/src/main.ts` (uses `foldkit/fieldValidation`; see the Form Validation section in Phase 4)

**Tier 4: URL routing, multiple pages, query parameters:**
Read `examples/routing/src/main.ts` and `examples/query-sync/src/main.ts`

**Tier 5: Complex state, nested domain models, CRUD, drag-and-drop:**
Read `examples/shopping-cart/src/main.ts` (nested domain schemas, cart state) and `examples/kanban/src/main.ts` (CRUD with `DragAndDrop`, flags restoring from localStorage, subscriptions)

**Tier 6: Submodels, OutMessage, multi-step forms, auth flows, multi-module apps:**
Read `examples/auth/src/main.ts` (login/signup with Submodels, OutMessage, protected routes) and `examples/job-application/src/main.ts` (multi-step form with deeply nested Submodels in `step/`, `DatePicker`, `FileDrop`, `Listbox`, `Calendar` module for date handling)

**Tier 7: Real-time, WebSocket, Managed Resources, production-grade:**
Read `packages/typing-game/client/src/update.ts`, then explore its `page/home/` and `page/room/` directories for the full Submodel/OutMessage pattern.
...
- `examples/ui-showcase/src/main.ts`: root wiring, `Got*` delegation, `toParentMessage` helpers
- `examples/ui-showcase/src/ui/message.ts`: how component Messages are structured
- `examples/ui-showcase/src/ui/model.ts`: how component Models are composed
- `examples/ui-showcase/src/ui/update.ts`: how component updates are delegated
- `examples/ui-showcase/src/ui/subscriptions.ts`: which components need Subscriptions lifted into the parent (`DragAndDrop`, `Slider`, `VirtualList`)
- `examples/ui-showcase/src/ui/toast.ts`: read when using `Toast`. It's unique in that it's parameterized on a payload schema via `Toast.make(PayloadSchema)`, returning a typed module you import from
...
### Ensure future Foldkit repository access

If OpenCode's `@foldkit` project reference is available, repository access is already configured. Do not offer or add a subtree.

Otherwise, offer to vendor Foldkit as a git subtree so future AI sessions can reference the full source, examples, and docs directly from the user's project. Commit the scaffold first so subtree has a base commit to merge into:
...
This fallback is optional but strongly recommended when no managed repository reference exists. The scaffolded `AGENTS.md` includes a `subtree_prompted: false` line that agents check on future sessions. If the subtree is absent and this flag is false, the agent offers to add it. Handling it up front here means the user's next AI session already has full context. If the user declines, update the line to `subtree_prompted: true` so they aren't asked again.
...
- For apps with multiple domain entities referenced across modules, extract shared schemas into `src/domain/` (e.g., `domain/product.ts`, `domain/session.ts`). See the shopping-cart and auth examples for this pattern, and read `packages/website/src/page/projectOrganization.ts` for guidance on when and how to structure domain modules
...
Canonical reference: `examples/form/src/main.ts` (async email uniqueness check with version-based cancellation) and `examples/job-application/src/step/` (validated multi-step forms across submodels).
...
- Canonical reference: `examples/job-application/src/step/attachments.ts`
...
- `examples/weather/src/story.test.ts`: simple Command resolution (happy path + error path)
- `examples/auth/src/page/loggedOut/page/login.story.test.ts`: Submodel with OutMessage assertions, field validation
- `packages/website/src/search/story.test.ts`: multi-step interactions (arrow key cycling, stale result handling)
...
- `examples/weather/src/scene.test.ts`: basic Scene flow with form interaction and Command resolution
- `examples/auth/src/scene.test.ts`: a multi-page app's root-level Scene driving the login flow through the root view
- `examples/kanban/src/scene.test.ts`: scoped queries with `within`, `toHaveValue`, explicit test data
...
2. architecture.md beside this skill's SKILL.md
3. conventions.md beside this skill's SKILL.md
4. checklist.md beside this skill's SKILL.md
5. blindSpots.md beside this skill's SKILL.md
6. At least one exemplar file matching the generated app's complexity,
   from the resolved Foldkit repository root (`@foldkit` first in
   OpenCode, then repos/foldkit/ when vendored):
...
Every path above is either project-relative, relative to this skill
directory, or relative to the resolved Foldkit repository root. In
OpenCode, use the advertised local path for `@foldkit`; do not rewrite
repository paths to `repos/foldkit/` when that reference is available.
